...
#ifdef __MWERKS__
// CW Pro 4 has optimization bugs with this file
	...
	loop_flags:	/* LOOP FOR \0, %, FLAGS, WIDTH, PRECISION, BASE, TYPE */
		...
		{ get_val:
			...
			{ case SFFMT_INT:
			 ...
			 case SFFMT_FLOAT:
#if !_ast_fltmax_double
				if(SFDOUBLE_T_FORCE || FMTCMP(size, Sfdouble_t))
		...
		case 'n':	/* return current output length */
			...
			else if(sizeof(short) < sizeof(int) && FMTCMP(size,short))
...
#endif
			if(sizeof(short) < sizeof(int) && FMTCMP(size,short))
		...
		case 'f':
			...
			flags = (flags & ~SFFMT_ZERO) | SFFMT_GFORMAT;
		...
		e_format: /* build the x.yyyy string */
			...
			*--ep = isupper(fmt) ? 'E' : 'e';
			...
			flags = (flags&~SFFMT_ZERO)|SFFMT_FLOAT;
			goto end_efg;
		...
		end_efg:
			...
			{	/* if a %gG, output the sign now */
				if(flags&SFFMT_GFORMAT)
				{	*--sp = '-';
					flags &= ~(SFFMT_SIGN|SFFMT_BLANK);
				}
				else	flags |= SFFMT_MINUS;
			}
			break;
		}
