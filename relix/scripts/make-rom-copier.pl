...
my $build_config_name = join '-', $arches{$arch}, $runtimes{$runtime}, 'blue', 'dbg';
...
my $relix_files_dir = "$ENV{HOME}/src/tree/metamage_1/relix/files";
...
if ( defined $ENV{BUILD_DATE} )
{
	...
	$vers_2_data = `vers "" "MacRelix ROM copier"`;
}
...
(
	...
	[
		# Standard
		\ qw( cat echo false ln sh sync true ),
	],
	etc =>
	[
		{
...
my $installer = "$lamp_dist/MacRelix ROM Copier";
my $archive   = "$lamp_dist/MacRelix-ROM-copier.mbin";
