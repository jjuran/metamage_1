...
static
void CloseUserWindow( const vfs::node* key )
{
...
static
bool port_has_window( const vfs::node* key )
{
	return gWindowParametersMap[ key ].itsWindow.get() != NULL;
}

static
void CreateUserWindow( const vfs::node* key )
{
...
static
void InvalidateWindowRef( WindowRef window )
{
	ASSERT( window != NULL );
...
static
WindowRef GetWindowRef( const vfs::node* key )
{
	if ( WindowParameters* it = gWindowParametersMap.find( key ) )
	{
		return it->itsWindow;
...
static
void focus_remove( const vfs::node* that )
{
...
static
void unfocus_symlink( const vfs::node*     that,
...
static
vfs::node_ptr focus_resolve( const vfs::node* that )
{
...
static
void window_touch( const vfs::node* that )
{
	invalidate_port_WindowRef( that->owner() );
}

static
void window_remove( const vfs::node* that )
{
...
static
plus::string window_readlink( const vfs::node* that )
{
...
static
vfs::filehandle_ptr port_tty_open( const vfs::node* that, int flags, mode_t mode );

static
void port_tty_attach( const vfs::node* that, const vfs::node* target )
{
...
static
void destroy_port_tty( vfs::filehandle* that )
{
...
static
int LookupGesture( const plus::string& name )
{
...
static
void gesture_remove( const vfs::node* that )
{
...
static
void ungesture_symlink( const vfs::node*     that,
...
static
plus::string gesture_readlink( const vfs::node* that )
{
	const vfs::node* view = that->owner();
...
static
WindowParameters& Find( const vfs::node* key )
{
	WindowParameters* it = gWindowParametersMap.find( key );
...
static
void destroy_lock_handle( vfs::filehandle* that )
{
...
static
vfs::filehandle_ptr lock_open( const vfs::node* that, int flags, mode_t mode )
{
...
static
void unwindow_touch( const vfs::node* that )
{
	CreateUserWindow( that->owner() );
}

static
vfs::filehandle_ptr unwindow_open( const vfs::node* that, int flags, mode_t mode )
{
	if ( ! (flags & O_CREAT) )
	{
		p7::throw_errno( ENOENT );
...
static
vfs::node_ptr new_lock( const vfs::node*     parent,
...
static
vfs::node_ptr new_window( const vfs::node*     parent,
                          const plus::string&  name,
                          const void*          args )
{
...
static
vfs::node_ptr new_focus( const vfs::node*     parent,
                         const plus::string&  name,
                         const void*          args )
{
...
static
vfs::node_ptr new_gesture( const vfs::node*     parent,
                           const plus::string&  name,
                           const void*          args )
{
...
static
vfs::node_ptr new_tty( const vfs::node*     parent,
                       const plus::string&  name,
                       const void*          args )
{
...
static
vfs::node_ptr new_port_property( const vfs::node*     parent,
                                 const plus::string&  name,
                                 const void*          params_ )
{
