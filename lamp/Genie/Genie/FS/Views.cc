...
static inline
const ViewParameters* find_view( const vfs::node* parent )
{
...
static
boost::intrusive_ptr< Ped::View > make_view( const vfs::node* parent )
{
...
static
const vfs::node_ptr& GetViewDelegate( const vfs::node* view )
{
...
static
void new_view_hardlink( const vfs::node*  that,
                        const vfs::node*  dest );
...
static
void unview_mkdir( const vfs::node* that, mode_t mode )
{
...
static
void view_touch( const vfs::node* that )
{
	InvalidateWindowForView( that );
}

static
void view_remove( const vfs::node* that )
{
	const view_extra& extra = *(view_extra*) that->extra();
...
static
vfs::node_ptr view_lookup( const vfs::node*     that,
                           const plus::string&  name,
                           const vfs::node*     parent )
{
...
static
void view_listdir( const vfs::node*    that,
