...
int mbsnwidth( const char *buf, size_t nbytes, int flags )
{
	int columns = 0;
	...
	while ( buf < end )
	{
		/* Assume UTF-8.  Continuation bytes are 10xxxxxx */
		
		if ( (*buf & 0xC0) != 0x80 )
		{
			/* Assume single-column characters */
			++columns;
		}
      ...
      if (MB_CUR_MAX > 1)
        {
          ...
          while (p < plimit)
          {
            /*
               Assume UTF-8.
               Assume that all sequences are valid.
               Assume that all multibyte characters are printable.
               Assume that all multibyte characters occupy a single column.
            */
            char c = *p++;
            
            displayed_width += (c & 0xC0) != 0x80;
