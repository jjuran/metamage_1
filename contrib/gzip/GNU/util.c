...
/*
 * Changed: August 21, 1995. spd for MacGzip 1.0
...
#ifndef MACGZIP /* spd *//* there is no paths in MacGzip */
/* ========================================================================
...
/* ========================================================================
 * Make name a legal MacOS file name
 */
    ...
    const char *m;
{
    ...
    fprintf(stderr, "\n%s: %s: %s\n", progname, ifname, m);
#else
	DoError( NO_ERR, WARN_ERR, "%s: %s: %s", progname, ifname, m);
#endif
    abort_gzip();
}
...
void read_error()
{
	...
	{
		DoError( NO_ERR, WARN_ERR, "%s: %s: unexpected end of file", progname, ifname);
	}
...
void write_error()
{
...
#else
	/*
	 * when the user cancels, or creation of ouput file fails,
	 * this is reported by MacIO as a write error.
	 * (in MB mode we can't create the output file till
	 * write has not started)
	 * So we must filter error codes...
	 */
	...
	switch ( errno )
	{
		...
		/* FSpClose */
			...
			DoError( STDC_ERR, WARN_ERR, "%s: %s", progname, ofname);
			break;
    ...
    FILE *file;
{
	...
	DoError( NO_ERR, INFO_ERR, "%2ld.%1ld%%", ratio / 10L, ratio % 10L);
